...
class SYGP:
	...
	def __init__(self, operators=[("+", 2), ("-", 2), ("*", 2), ("/", 2)], max_initial_depth=6, population_size=1,
		max_generation=100, max_evaluation=50000, tournament_size=5, elitism_size=1, max_depth=17,
		dim_min=1, dim_max=9999, threads=1, random_state=42, update_all=False, crossover_type=0, validation=True,kfold=5, verbose=True, model_name="MahalanobisDistanceClassifier", fitnessType="Accuracy", dataset="heart.csv", max_niche_size=300, max_niche_count=30,   method_name="",n2complexity=0,initial_random=1, classification=True,k_split_trainingset=False, base1=1,base2=1,cross_rate=1):
	...
	def fit(self,Tr_x, Tr_y, Te_x = None, Te_y = None, V_x=None, V_y=None):
		...
		self.IndividualInsert(self.population_size, 0, 0)
		...
		if randomPop :
			...
			for i in range(len(self.allPopulation)):
				...
				if self.OnePop:
					...
					j=i
				offspring, id1, id2, iscross, p1, p2 = getOffspring(self.MSF,self.pool_population,self.rng, self.allPopulation, j, numGeneration, self.tournament_size, self.Tr_x, self.Tr_y, self.crossover_type, idd, self.operators, self.terminals, self.max_depth, self.model_name, self.fitnessType,0,self.cross_rate)
	...
	def nextGeneration(self, numGeneration):
		...
		if self.verbose and self.currentGeneration % 1 == 0:
			...
			print(
			"> Gen #%2d:  Fitness: %.6f //train: %.6f //test: %.6f//dim: %.6f // eval: %.6f // Time: %.4f" % (
	...
	def synergyByReplacement(self, offspring):
		f1=self.msfvector.getFitnesses(self.Tr_x, self.Tr_y)
