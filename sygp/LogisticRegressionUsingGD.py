...
class LogisticRegressionUsingGD:
    ...
    def cost_function(self, theta, x, y):
        ...
        if regularization:
            ...
            total_cost = (-y * np.log(h) - (1 - y) * np.log(1 - h)).mean() + lambda1 / (2 * m) * sum(theta ** 2)
        else:
    ...
    def predict(self, x):
        ...
        predicted_classes = (self.probability(theta, x) >= probab_threshold).astype(int)
